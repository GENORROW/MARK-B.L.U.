...
class IdentityDatabase:
    """Centralized database for managing agent identities."""
    
    def __init__(self, db_path: str = "data/agent_identities.db"):
    ...
    def store_identity(self, identity: AgentIdentity) -> bool:
        """Store an agent identity in the database."""
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("""
    ...
    def store_multiple_identities(self, identities: List[AgentIdentity]) -> Tuple[int, int]:
        """Store multiple agent identities."""
        successful = 0
        failed = 0
    ...
    def get_identity(self, serial: str, timeslot: int) -> Optional[AgentIdentity]:
        """Retrieve a specific agent identity."""
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
    ...
    def get_current_identity(self, serial: str) -> Optional[AgentIdentity]:
        """Get the most recent identity for an agent."""
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
                SELECT serial, timeslot, timestamp, qrng_seed, badge
    ...
    def get_all_identities_for_timeslot(self, timeslot: int) -> List[AgentIdentity]:
        """Get all agent identities for a specific timeslot."""
        identities = []
    ...
    def log_communication(self, sender_serial: str, receiver_serial: str,
        ...
        Args:
            sender_serial: Sender agent serial
            receiver_serial: Receiver agent serial
    ...
    def update_fleet_metadata(self, fleet_id: str, num_agents: int, current_timeslot: int):
        """Update fleet metadata."""
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO fleet_metadata (fleet_id, num_agents, current_timeslot)
                VALUES (?, ?, ?)
                ON CONFLICT(fleet_id) DO UPDATE SET
                    num_agents = excluded.num_agents,
                    current_timeslot = excluded.current_timeslot,
                    last_updated = CURRENT_TIMESTAMP
            """, (fleet_id, num_agents, current_timeslot))
            conn.commit()
...
def demo_database():
    ...
    test_agents = []
    for i in range(3):
        agent = Agent(f"TEST-AGENT-{i+1:03d}")
    ...
    for timeslot in range(1, 3):
        ...
        for agent in test_agents:
            identity = agent.generate_identity(timeslot)
    ...
    # Query database
    print("\n[*] Querying database...")
    current_id = db.get_current_identity("TEST-AGENT-001")
    if current_id:
        print(f"[✓] Current identity for TEST-AGENT-001:")
